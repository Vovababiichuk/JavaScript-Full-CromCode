...
const onToggleTask = e => {
  ...
  if (taskData.done) {
    taskData.completedDate = new Date();  // оновлюємо дату виконання задачі
  } else {
    delete taskData.completedDate;  // видаляємо дату виконання, якщо задача відмічена як невиконана
  }
  renderTasks(tasks);
};
...
const onCreateTask = () => {
  ...
  if (!text) {
    return;  // Не додаємо пусті задачі
  }
  taskTitleInputElem.value = '';  // Очищуємо поле вводу
